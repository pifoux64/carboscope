...
<?php if($boxed_layout==1): ?>
	<div class="ilist-box-style-main "><!--ilist-box-style-main -->
		<div class="ilist-box-header box-shadow">
			<div class="ilist-border-style-top"><!--ilist-border-style-top-->
				<span class="ilist-border ilist-border-01"></span>
				<span class="ilist-border ilist-border-02"></span>
				<span class="ilist-border ilist-border-03"></span>
				<span class="ilist-border ilist-border-04"></span>
			</div><!--end ilist-border-style-top-->
			
			
			<div class="ilist-headline">
				<?php 
				if($main_title==1){
				?>
				<h2>
...
<div id="qcopd-list-<?php echo $listId; ?>" >
		<div class="qcopd-single-list">
			...
			?>
	<?php 
	if($main_title==1 and $boxed_layout==0){
	?>
	...
	<div style="clear:both;margin-bottom:20px"></div>
		<?php
	if(isset($ilist_chart[0]) && $ilist_chart[0]!='' and $show_chart_position[0]=='top'){
	?>
	...
	<?php foreach($list_sl as $list) : ?>
		<li id="qcld_sl_<?php echo get_the_ID()."_".$cnt ?>" class="list-style-seventeen list-style-seventeen-01 ilist_pinfo5_<?php echo $column ?>">
			<div class="list-seventeen-inner-part pinfo5hoveritem ori10itembgcolor" style="<?php echo ($list['qcld_entrybg_color']!=''?'background:'.$list['qcld_entrybg_color']:''); ?>">
				...
				<div class="list-seventeen-content pinfo5hoverdesccolor pinfo5hovertitlecolor">
					<?php 
						...
						?>
							<h2 <?php echo $titlecolor; ?> class="pinfo5titlefont pinfog5tfontsize" >
								<?php 
										echo ($list['qcld_text_title']);
								?>
				<?php if( $upvote == 'on' ) : ?>
				<div class="upvote-section" style="display: inline-block;margin-left: 10px;cursor: pointer;">
					<span data-post-id="<?php echo get_the_ID(); ?>" data-item-title="<?php echo trim($list['qcld_text_title']); ?>" class="upvote-btn-ilist upvote-on" style="line-height: 48px;">
		...
		<?php
	if(isset($ilist_chart[0]) && $ilist_chart[0]!='' and $show_chart_position[0]=='bottom'){
	?>
	<div style="width:60%;margin:0 auto">
		<?php echo do_shortcode($ilist_chart[0]);?>
	</div>
...
<?php if($boxed_layout==1): ?> </div> <?php endif; ?>
